...
import com.airepublic.microprofile.module.http.websocket.util.ExceptionUtils;
import com.airepublic.microprofile.module.http.websocket.util.buf.Utf8Decoder;
import com.airepublic.microprofile.module.http.websocket.util.res.StringManager;

/**
 * Takes the ServletInputStream, processes the WebSocket frames it contains and extracts the
 * messages. WebSocket Pings received will be responded to automatically without any action required
 * by the application.
 */
public abstract class WsFrameBase {
            ...
            if (continuationExpected) {
                ...
                try {
                    if (opCode == Constants.OPCODE_BINARY) {
                        // New binary message
                        textMessage = false;
                        final int size = wsSession.getMaxBinaryMessageBufferSize();
                        ...
                        // New text message
                        textMessage = true;
                        final int size = wsSession.getMaxTextMessageBufferSize();
    ...
    /**
     * @return <code>true</code> if sufficient data was present to complete the processing of the
     *         header
     */
    ...
    private boolean processDataControl() throws IOException {
        final TransformationResult tr = transformation.getMoreData(opCode, fin, rsv, controlBufferBinary);
        ...
        if (opCode == Constants.OPCODE_CLOSE) {
            ...
            if (controlBufferBinary.remaining() > 1) {
                ...
                if (controlBufferBinary.remaining() > 0) {
                    final CoderResult cr = utf8DecoderControl.decode(controlBufferBinary, controlBufferText, true);
            ...
            final MessageHandler.Whole<PongMessage> mhPong = wsSession.getPongMessageHandler();
            if (mhPong != null) {
                try {
    ...
    protected void sendMessageText(final boolean last) throws WsIOException {
        if (textMsgHandler instanceof WrappedMessageHandler) {
            final long maxMessageSize = ((WrappedMessageHandler) textMsgHandler).getMaxMessageSize();
    ...
    private boolean processDataText() throws IOException {
        ...
        while (!TransformationResult.END_OF_FRAME.equals(tr)) {
            ...
            while (true) {
                final CoderResult cr = utf8DecoderMessage.decode(messageBufferBinary, messageBufferText, false);
        ...
        // Convert bytes to UTF-8
        while (true) {
            final CoderResult cr = utf8DecoderMessage.decode(messageBufferBinary, messageBufferText,
                    last);
            if (cr.isError()) {
                throw new WsIOException(new CloseReason(
    ...
    private boolean processDataBinary() throws IOException {
        ...
        while (!TransformationResult.END_OF_FRAME.equals(tr)) {
            ...
            if (!usePartial()) {
                final CloseReason cr = new CloseReason(CloseCodes.TOO_BIG,
            ...
            messageBufferBinary.flip();
            final ByteBuffer copy = ByteBuffer.allocate(messageBufferBinary.limit());
    ...
    private void handleThrowableOnSend(final Throwable t) throws WsIOException {
        ...
        final CloseReason cr = new CloseReason(CloseCodes.CLOSED_ABNORMALLY,
    ...
    protected void sendMessageBinary(final ByteBuffer msg, final boolean last) throws WsIOException {
        if (binaryMsgHandler instanceof WrappedMessageHandler) {
            final long maxMessageSize = ((WrappedMessageHandler) binaryMsgHandler).getMaxMessageSize();
    ...
    private boolean swallowInput() {
        final long toSkip = Math.min(payloadLength - payloadWritten, inputBuffer.remaining());
    ...
    /**
     * WAITING - not suspended Server case: waiting for a notification that data is ready to be read
     * from the socket, the socket is registered to the poller Client case: data has been read from
     * the socket and is waiting for data to be processed PROCESSING - not suspended Server case:
     * reading from the socket and processing the data Client case: processing the data if such has
     * already been read and more data will be read from the socket SUSPENDING_WAIT - suspended, a
     * call to suspend() was made while in WAITING state. A call to resume() will do nothing and
     * will transition to WAITING state SUSPENDING_PROCESS - suspended, a call to suspend() was made
     * while in PROCESSING state. A call to resume() will do nothing and will transition to
     * PROCESSING state SUSPENDED - suspended Server case: processing data finished
     * (SUSPENDING_PROCESS) / a notification was received that data is ready to be read from the
     * socket (SUSPENDING_WAIT), socket is not registered to the poller Client case: processing data
     * finished (SUSPENDING_PROCESS) / data has been read from the socket and is available for
     * processing (SUSPENDING_WAIT) A call to resume() will: Server case: register the socket to the
     * poller Client case: resume data processing CLOSING - not suspended, a close will be send
     *
     * <pre>
    ...
    protected enum ReadState {
        WAITING(false), PROCESSING(false), SUSPENDING_WAIT(true), SUSPENDING_PROCESS(true), SUSPENDED(true), CLOSING(false);

        private final boolean isSuspended;


        ReadState(final boolean isSuspended) {
    ...
    /**
     * For use by the client implementation that needs to obtain payload data without the need for
     * unmasking.
     */
    private final class NoopTransformation extends TerminalTransformation {

        @Override
        public TransformationResult getMoreData(final byte opCode, final boolean fin, final int rsv,
            ...
            final int orgLimit = inputBuffer.limit();
    ...
    /**
     * For use by the server implementation that needs to obtain payload data and unmask it before
    ...
    private final class UnmaskTransformation extends TerminalTransformation {
        ...
        public TransformationResult getMoreData(final byte opCode, final boolean fin, final int rsv,
            ...
            while (payloadWritten < payloadLength && inputBuffer.remaining() > 0 &&
                    dest.hasRemaining()) {
                final byte b = (byte) ((inputBuffer.get() ^ mask[maskIndex]) & 0xFF);
                maskIndex++;
                if (maskIndex == 4) {
                    maskIndex = 0;
                }
