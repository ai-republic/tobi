...
import com.airepublic.microprofile.module.http.websocket.util.buf.Utf8Encoder;
...
public abstract class WsRemoteEndpointImplBase implements RemoteEndpoint {
    ...
    public Future<Void> sendBytesByFuture(final ByteBuffer data) {
        final FutureToSendHandler f2sh = new FutureToSendHandler(wsSession);
    ...
    public void sendBytesByCompletion(final ByteBuffer data, final SendHandler handler) {
        ...
        final StateUpdateSendHandler sush = new StateUpdateSendHandler(handler, stateMachine);
        stateMachine.binaryStart();
        startMessage(Constants.OPCODE_BINARY, data, true, sush);
    }


    public void sendPartialBytes(final ByteBuffer partialByte, final boolean last)
            throws IOException {
        if (partialByte == null) {
            throw new IllegalArgumentException(sm.getString("wsRemoteEndpoint.nullData"));
        }
    ...
    public void sendStringByCompletion(final String text, final SendHandler handler) {
        ...
        stateMachine.textStart();
        final TextMessageSendHandler tmsh = new TextMessageSendHandler(handler,
    ...
    void sendMessageBlock(final CharBuffer part, final boolean last) throws IOException {
        final long timeoutExpiry = getTimeoutExpiry();
        boolean isDone = false;
        while (!isDone) {
            encoderBuffer.clear();
            final CoderResult cr = encoder.encode(part, encoderBuffer, true);
            if (cr.isError()) {
    ...
    private long getTimeoutExpiry() {
        ...
        final long timeout = getBlockingSendTimeout();
        if (timeout < 0) {
    ...
    private void sendMessageBlock(final byte opCode, final ByteBuffer payload, final boolean last, final long timeoutExpiry) throws IOException {
        ...
        try {
            if (!messagePartInProgress.tryAcquire(timeout, TimeUnit.MILLISECONDS)) {
                final String msg = sm.getString("wsRemoteEndpoint.acquireTimeout");
            ...
            final String msg = sm.getString("wsRemoteEndpoint.sendInterrupt");
            wsSession.doClose(new CloseReason(CloseCodes.GOING_AWAY, msg), new CloseReason(CloseCodes.CLOSED_ABNORMALLY, msg));
    ...
    void startMessage(final byte opCode, final ByteBuffer payload, final boolean last,
    ...
    void writeMessagePart(final MessagePart mp) {
        ...
        if (Constants.INTERNAL_OPCODE_FLUSH == mp.getOpCode()) {
            ...
            nextText = text;
            outputBuffer.flip();
            final SendHandler flushHandler = new OutputBufferFlushSendHandler(
        ...
        if (getBatchingAllowed() || isMasked()) {
            ...
            final OutputBufferSendHandler obsh = new OutputBufferSendHandler(mp.getEndHandler(), mp.getBlockingWriteTimeoutExpiry(), headerBuffer, mp.getPayload(), mask, outputBuffer, !getBatchingAllowed(), this);
            obsh.write();
        } else {
            // Can write directly
    ...
    private long getBlockingSendTimeout() {
        final Object obj = wsSession.getUserProperties().get(Constants.BLOCKING_SEND_TIMEOUT_PROPERTY);
    ...
    /**
     * Wraps the user provided handler so that the end point is notified when the message is
     * complete.
     */
    ...
    /**
     * If a transformation needs to split a {@link MessagePart} into multiple {@link MessagePart}s,
     * it uses this handler as the end handler for each of the additional {@link MessagePart}s. This
     * handler notifies this this class that the {@link MessagePart} has been processed and that the
     * next {@link MessagePart} in the queue should be started. The final {@link MessagePart} will
     * use the {@link EndMessageHandler} provided with the original {@link MessagePart}.
     */
    ...
    public void sendObject(final Object obj) throws IOException, EncodeException {
        ...
        /*
         * Note that the implementation will convert primitives and their object equivalents by
         * default but that users are free to specify their own encoders and decoders for this if
         * they wish.
         */
        final Encoder encoder = findEncoder(obj);
        ...
        if (encoder == null && byte[].class.isAssignableFrom(obj.getClass())) {
            final ByteBuffer msg = ByteBuffer.wrap((byte[]) obj);
            sendBytes(msg);
            return;
        }
        ...
        if (encoder instanceof Encoder.Text) {
            ...
            final ByteBuffer msg = ((Encoder.Binary) encoder).encode(obj);
            sendBytes(msg);
    ...
    public void sendObjectByCompletion(final Object obj, final SendHandler completion) {
        ...
        try {
            ...
            final SendResult sr = new SendResult(e);
    ...
    protected void setEncoders(final EndpointConfig endpointConfig)
        ...
        for (final Class<? extends Encoder> encoderClazz : endpointConfig.getEncoders()) {
            ...
            final EncoderEntry entry = new EncoderEntry(
    ...
    private Encoder findEncoder(final Object obj) {
        for (final EncoderEntry entry : encoderEntries) {
    ...
    private static void writeHeader(final ByteBuffer headerBuffer, final boolean fin,
            final int rsv, final byte opCode, final boolean masked, final ByteBuffer payload,
            final byte[] mask, final boolean first) {
    ...
    private class TextMessageSendHandler implements SendHandler {
        ...
        public void write() {
            buffer.clear();
            final CoderResult cr = encoder.encode(message, buffer, true);
            if (cr.isError()) {
                throw new IllegalArgumentException(cr.toString());
            }
            isDone = !cr.isOverflow();
        ...
        public void onResult(final SendResult result) {
            if (isDone) {
                ...
                final SendResult sr = new SendResult(new IOException(
    ...
    private static class OutputBufferSendHandler implements SendHandler {
        ...
        public OutputBufferSendHandler(final SendHandler completion,
                ...
                final ByteBuffer headerBuffer, final ByteBuffer payload, final byte[] mask,
                final ByteBuffer outputBuffer, final boolean flushRequired,
                final WsRemoteEndpointImplBase endpoint) {
        ...
        public void write() {
            ...
            // Write the payload
            final int payloadLeft = payload.remaining();
            final int payloadLimit = payload.limit();
            final int outputSpace = outputBuffer.remaining();
    ...
    private static class StateMachine {
        ...
        private void checkState(final State... required) {
            ...
            throw new IllegalStateException(
                    sm.getString("wsRemoteEndpoint.wrongState", state));
        }
    }
