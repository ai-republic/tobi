...
public class ResteasyInjectorFactory implements InjectorFactory {
    @Override
    public ConstructorInjector createConstructor(final Constructor constructor, final ResteasyProviderFactory providerFactory) {
    ...
    @Override
    public PropertyInjector createPropertyInjector(final Class resourceClass, final ResteasyProviderFactory providerFactory) {
    ...
    public ValueInjector createParameterExtractor(final Parameter parameter, final ResteasyProviderFactory providerFactory) {
        // if (SseEventSink.class.isAssignableFrom(parameter.getType())) {
        // return new
        // }
        ...
        switch (parameter.getParamType()) {
            ...
            case FORM: {
                final String prefix = parameter.getParamName();
                if (prefix.length() > 0) {
                    if (parameter.getGenericType() instanceof ParameterizedType) {
                        final ParameterizedType pType = (ParameterizedType) parameter.getGenericType();
    ...
    public ValueInjector createParameterExtractor(final Class injectTargetClass, final AccessibleObject injectTarget, final String defaultName, final Class type,
            final Type genericType, final Annotation[] annotations, final ResteasyProviderFactory providerFactory) {
        final ValueInjector res = createParameterExtractor(injectTargetClass, injectTarget, type, genericType, annotations, false, providerFactory);
        if (res != null) {
            return res;
        } else {
            return createParameterExtractorUsingProprietaryAnnotations(injectTargetClass, injectTarget, defaultName, type, genericType, annotations, true, providerFactory);
        }
    }
    ...
    public ValueInjector createParameterExtractor(final Class injectTargetClass, final AccessibleObject injectTarget, final Class type,
        ...
        if (res != null) {
            ...
            return createParameterExtractorUsingProprietaryAnnotations(injectTargetClass, injectTarget, null, type, genericType, annotations, true, providerFactory);
        }
    }


    @Override
    public ValueInjector createParameterExtractor(final Class injectTargetClass, final AccessibleObject injectTarget, final String defaultName, final Class type, final Type genericType, final Annotation[] annotations, final boolean useDefault, final ResteasyProviderFactory providerFactory) {
        final ValueInjector res = createParameterExtractor(injectTargetClass, injectTarget, type, genericType, annotations, false, providerFactory);
        if (res != null) {
            return res;
        } else {
    ...
    @Override
    @Deprecated
    public ValueInjector createParameterExtractor(final Class injectTargetClass, final AccessibleObject injectTarget, final Class type, final Type genericType, final Annotation[] annotations, final boolean useDefault, final ResteasyProviderFactory providerFactory) {
        final DefaultValue defaultValue = findAnnotation(annotations, DefaultValue.class);
        final boolean encode = findAnnotation(annotations, Encoded.class) != null || injectTarget.isAnnotationPresent(Encoded.class) || type.isAnnotationPresent(Encoded.class);
        ...
        if ((queryParam = findAnnotation(annotations, QueryParam.class)) != null) {
            ...
            final String prefix = form.prefix();
            if (prefix.length() > 0) {
    ...
    private ValueInjector createParameterExtractorUsingProprietaryAnnotations(final Class injectTargetClass, final AccessibleObject injectTarget, final String defaultName, final Class type,
            final Type genericType, final Annotation[] annotations, final boolean useDefault, final ResteasyProviderFactory providerFactory) {
        final DefaultValue defaultValue = findAnnotation(annotations, DefaultValue.class);
        final boolean encode = findAnnotation(annotations, Encoded.class) != null || injectTarget.isAnnotationPresent(Encoded.class) || type.isAnnotationPresent(Encoded.class);
        String defaultVal = null;
        if (defaultValue != null) {
        ...
        if (findAnnotation(annotations, org.jboss.resteasy.annotations.jaxrs.QueryParam.class) != null) {
