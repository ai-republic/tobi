...
public class PublicKeyEndpoint {
    ...
    public JsonObject verifyKeyAsPEM() {
        ...
        final String issValue = issuer.orElse("missing-issuer");
        ...
        try {
            final PublicKey publicKey = SimpleTokenUtils.decodePublicKey(key.orElse("bad-key"));
            msg += " | key as PEM PASS";
            pass = true;
    ...
    public JsonObject verifyKeyLocationAsPEMResource() {
        ...
        if (location.isPresent()) {
            final String locationValue = location.get();
            log.info(String.format("verifyKeyLocationAsPEMResource, location=%s", locationValue));
            try {
                final String pemValue = SimpleTokenUtils.readResource(locationValue);
                log.info(String.format("verifyKeyLocationAsPEMResource, locationValue=%s", pemValue));
                final PublicKey publicKey = SimpleTokenUtils.decodePublicKey(pemValue);
    ...
    public JsonObject verifyKeyLocationAsPEMUrl() {
        ...
        if (location.isPresent()) {
            ...
            try {
                ...
                final URL locationURL = new URL(locationValue);
                final StringWriter pemContents = new StringWriter();
                ...
                // Decode the contents
                final PublicKey publicKey = SimpleTokenUtils.decodePublicKey(pemContents.toString());
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed("Tester")
    public JsonObject verifyKeyAsJWK(@QueryParam("kid") final String kid) {
        boolean pass = false;
        String msg;

        // Check that the key exists and is a valid JWKS public key
        try {
            final String jsonJwk = key.get();
            final StringBuilder msgBuilder = new StringBuilder();
            final JsonObject jwk = Json.createReader(new StringReader(jsonJwk)).readObject();
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed("Tester")
    public JsonObject verifyKeyAsBase64JWK(@QueryParam("kid") final String kid) {
        boolean pass = false;
        String msg;

        // Check that the key exists and is a valid base64 JWK public key
        try {
            final String base64Jwk = key.get();
            log.info("verifyKeyAsBase64JWK, base64Jwk=" + base64Jwk);
            final byte[] data = Base64.getDecoder().decode(base64Jwk);
            final String jsonJwk = new String(data);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed("Tester")
    public JsonObject verifyKeyAsJWKS(@QueryParam("kid") final String kid) {
        boolean pass = false;
        String msg;
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed("Tester")
    public JsonObject verifyKeyLocationAsJWKResource(@QueryParam("kid") final String kid) {
        boolean pass = false;
        String msg;
        // Check the location exists and is a valid PEM public key
        if (location.isPresent()) {
            ...
            try {
                final String jwkValue = SimpleTokenUtils.readResource(locationValue);
                log.info(String.format("verifyKeyLocationAsJWKResource, locationValue=%s", jwkValue));
                final StringBuilder msgBuilder = new StringBuilder();
                final JsonObject jwk = Json.createReader(new StringReader(jwkValue)).readObject();
                if (verifyJWK(jwk, kid, msgBuilder)) {
                    final PublicKey publicKey = SimpleTokenUtils.decodeJWKSPublicKey(jwkValue);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed("Tester")
    public JsonObject verifyKeyLocationAsJWKSResource(@QueryParam("kid") final String kid) {
        boolean pass = false;
        String msg;
        // Check the location exists and is a valid PEM public key
        if (location.isPresent()) {
            ...
            try {
                final String jwksValue = SimpleTokenUtils.readResource(locationValue);
                log.info(String.format("verifyKeyLocationAsJWKSResource, locationValue=%s", jwksValue));
                final StringBuilder msgBuilder = new StringBuilder();
                if (verifyJWKS(jwksValue, kid, msgBuilder)) {
                    final PublicKey publicKey = SimpleTokenUtils.decodeJWKSPublicKey(jwksValue);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @RolesAllowed("Tester")
    public JsonObject verifyKeyLocationAsJWKSUrl(@QueryParam("kid") final String kid) {
        boolean pass = false;
        String msg;
        if (location.isPresent()) {
            final String locationValue = location.get();
            ...
            try {
                // Read the pem contents from the URL
                final URL locationURL = new URL(locationValue);
                final StringWriter jwksContents = new StringWriter();
                try (BufferedReader reader = new BufferedReader(new InputStreamReader(locationURL.openStream()))) {
                    String line = reader.readLine();
                    while (line != null) {
                ...
                if (verifyJWKS(jwksContents.toString(), kid, msgBuilder)) {
                    final PublicKey publicKey = SimpleTokenUtils.decodeJWKSPublicKey(jwksContents.toString());
                    log.info(String.format("verifyKeyLocationAsJWKSResource, publicKey=%s", publicKey));
    ...
    public JsonObject verifyBadIssIsOk() {
        ...
        if (!iss.getValue().isPresent()) {
            ...
            final String claimIss = iss.getValue().get();
            final String configIss = issuer.get();
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @PermitAll
    public JsonObject publicKey4kAsJWKS(@QueryParam("kid") final String kid) throws Exception {
        final String pem = SimpleTokenUtils.readResource("/publicKey4k.pem");
        final RSAPublicKey publicKey = (RSAPublicKey) SimpleTokenUtils.decodePublicKey(pem);
        final JsonObjectBuilder jwksBuilder = Json.createObjectBuilder();
        final JsonObjectBuilder keyBuilder = Json.createObjectBuilder();
        final BigInteger nBI = publicKey.getModulus();
        byte[] nbytes = nBI.toByteArray();
        if (nBI.bitLength() % 8 == 0 && nbytes[0] == 0 && nbytes.length > 1) {
            final byte[] tmp = new byte[nbytes.length - 1];
            System.arraycopy(nbytes, 1, tmp, 0, tmp.length);
            nbytes = tmp;
        }
        final String n = new String(Base64.getUrlEncoder().withoutPadding().encode(nbytes));
        final BigInteger eBI = publicKey.getPublicExponent();
        ...
        final String e = new String(Base64.getUrlEncoder().withoutPadding().encode(ebytes));
        ...
        final JsonObject jwks = jwksBuilder.build();
        return jwks;
    }
     ...
     */
    private boolean verifyJWKS(final String jwksJson, final String kid, final StringBuilder msg) {
        boolean pass;
        final JsonObject jwks = Json.createReader(new StringReader(jwksJson)).readObject();
        final JsonArray keys = jwks.getJsonArray("keys");
        if (keys != null) {
            final JsonObject key = keys.getJsonObject(0);
            final StringBuilder msgBuilder = new StringBuilder();
    ...
    private boolean verifyJWK(final JsonObject key, final String kid, final StringBuilder msg) {
