...
/**
 * Test cases for CDI-based API that test retrieving values from the configuration. The tests depend
 * only on CDI 1.2.
 * 
...
public class CDIPlainInjectionTest extends Arquillian {
    ...
    public void canInjectSimpleValuesWhenDefined() {
        final SimpleValuesBean bean = getBeanOfType(SimpleValuesBean.class);
    ...
    public void canInjectDynamicValuesViaCdiProvider() {
        ...
        final DynamicValuesBean bean = getBeanOfType(DynamicValuesBean.class);
    ...
    public void canInjectDefaultPropertyPath() {
        final DefaultPropertyBean bean = getBeanOfType(DefaultPropertyBean.class);
    ...
    private <T> T getBeanOfType(final Class<T> beanClass) {
