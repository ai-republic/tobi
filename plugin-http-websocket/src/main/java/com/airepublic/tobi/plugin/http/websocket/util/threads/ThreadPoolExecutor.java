 ...
 */
package com.airepublic.tobi.plugin.http.websocket.util.threads;
 ...
 * {@link #getSubmittedCount()} method, to be used to properly handle the work queue. If a
 * RejectedExecutionHandler is not specified a default one will be configured and that one will
 * always throw a RejectedExecutionException
 *
 */
public class ThreadPoolExecutor extends java.util.concurrent.ThreadPoolExecutor {
    ...
    /**
     * The number of tasks submitted but not yet finished. This includes tasks in the queue and
     * tasks that have been handed to a worker thread but the latter did not start executing the
     * task yet. This number is always greater or equal to {@link #getActiveCount()}.
     */
    ...
    /**
     * If the current thread was started before the last time when a context was stopped, an
     * exception is thrown so that the current thread is stopped.
     */
    protected void stopCurrentThreadIfNeeded() {
        if (currentThreadShouldBeStopped()) {
            final long lastTime = lastTimeThreadKilledItself.longValue();
     ...
     * <b>Executor</b> implementation. If no threads are available, it will be added to the work
     * queue. If the work queue is full, the system will wait for the specified time and it throw a
     * RejectedExecutionException if the queue is still full after that.
     *
     * @param command the runnable task
     * @param timeout A timeout for the completion of the task
     * @param unit The timeout time unit
     * @throws RejectedExecutionException if this task cannot be accepted for execution - the queue
     *         is full
    ...
    public void contextStopping() {
        ...
        final int savedCorePoolSize = getCorePoolSize();
        final TaskQueue taskQueue = getQueue() instanceof TaskQueue ? (TaskQueue) getQueue() : null;
        if (taskQueue != null) {
    ...
    private static class RejectHandler implements RejectedExecutionHandler {
        ...
        public void rejectedExecution(final Runnable r,
                final java.util.concurrent.ThreadPoolExecutor executor) {
