...
/**
 * Wraps the {@link AsynchronousSocketChannel} with SSL/TLS. This needs a lot more testing before it
 * can be considered robust.
 */
public class AsyncChannelWrapperSecure implements AsyncChannelWrapper {

    private final Logger log = LoggerFactory.getLogger(AsyncChannelWrapperSecure.class);
    ...
    public AsyncChannelWrapperSecure(final AsynchronousSocketChannel socketChannel,
    ...
    public Future<Integer> read(final ByteBuffer dst) {
        final WrapperFuture<Integer, Void> future = new WrapperFuture<>();
    ...
    @Override
    public <B, A extends B> void read(final ByteBuffer dst, final A attachment,
            final CompletionHandler<Integer, B> handler) {

        final WrapperFuture<Integer, B> future = new WrapperFuture<>(handler, attachment);
    ...
    public Future<Integer> write(final ByteBuffer src) {
        ...
        final WrapperFuture<Long, Void> inner = new WrapperFuture<>();
    ...
    @Override
    public <B, A extends B> void write(final ByteBuffer[] srcs, final int offset, final int length,
            final long timeout, final TimeUnit unit, final A attachment,
            final CompletionHandler<Long, B> handler) {

        final WrapperFuture<Long, B> future = new WrapperFuture<>(handler, attachment);
    ...
    public Future<Void> handshake() throws SSLException {
        ...
        final WrapperFuture<Void, Void> wFuture = new WrapperFuture<>();
    ...
    private class WriteTask implements Runnable {
        ...
        public WriteTask(final ByteBuffer[] srcs, final int offset, final int length,
                final WrapperFuture<Long, ?> future) {
            this.srcs = srcs;
            this.future = future;
            this.offset = offset;
            this.length = length;
            ...
            try {
                for (int i = offset; i < offset + length; i++) {
                    ...
                    while (src.hasRemaining()) {
                        ...
                        // Encrypt the data
                        final SSLEngineResult r = sslEngine.wrap(src, socketWriteBuffer);
                        written += r.bytesConsumed();
                        final Status s = r.getStatus();
                        ...
                        while (toWrite > 0) {
                            final Future<Integer> f = socketChannel.write(socketWriteBuffer);
                            final Integer socketWrite = f.get();
        ...
        public void run() {
            ...
            try {
                while (read == 0) {
                    ...
                    if (forceRead) {
                        forceRead = false;
                        final Future<Integer> f = socketChannel.read(socketReadBuffer);
                    ...
                    if (socketReadBuffer.hasRemaining()) {
                        ...
                        final SSLEngineResult r = sslEngine.unwrap(socketReadBuffer, dest);
    ...
    private class WebSocketSslHandshakeThread extends Thread {
            ...
            try {
                ...
                while (handshaking) {
                    switch (handshakeStatus) {
                        case NEED_WRAP: {
                            socketWriteBuffer.clear();
                            final SSLEngineResult r = sslEngine.wrap(DUMMY, socketWriteBuffer);
                            checkResult(r, true);
                            socketWriteBuffer.flip();
                            final Future<Integer> fWrite = socketChannel.write(socketWriteBuffer);
                            fWrite.get();
                            break;
                        }
                        case NEED_UNWRAP: {
                            ...
                            if (socketReadBuffer.position() == 0 ||
                                ...
                                final Future<Integer> fRead = socketChannel.read(socketReadBuffer);
                                fRead.get();
                            }
                            socketReadBuffer.flip();
                            final SSLEngineResult r = sslEngine.unwrap(socketReadBuffer, DUMMY);
                            checkResult(r, false);
                            break;
                        }
                        case NEED_TASK: {
        ...
        private void checkResult(final SSLEngineResult result, final boolean wrap)
                throws SSLException {
    ...
    private static class WrapperFuture<T, A> implements Future<T> {
        ...
        public T get(final long timeout, final TimeUnit unit)
                ...
                TimeoutException {
            final boolean latchResult = completionLatch.await(timeout, unit);
    ...
    private static final class LongToIntegerFuture implements Future<Integer> {
        ...
        public Integer get(final long timeout, final TimeUnit unit)
                throws InterruptedException, ExecutionException,
                TimeoutException {
            final Long result = wrapped.get(timeout, unit);
            if (result.longValue() > Integer.MAX_VALUE) {
                throw new ExecutionException(sm.getString(
                        "asyncChannelWrapperSecure.tooBig", result), null);
            }
