...
public class WsFrameClient extends WsFrameBase {
    ...
    public WsFrameClient(final ByteBuffer response, final AsyncChannelWrapper channel, final WsSession wsSession,
            final Transformation transformation) {
        super(wsSession, transformation);
        this.response = response;
        this.channel = channel;
        handler = new WsFrameClientCompletionHandler();
    }
            ...
            switch (getReadState()) {
                case WAITING:
                    ...
                    while (response.hasRemaining()) {
                        ...
                        final int toCopy = Math.min(response.remaining(), inputBuffer.remaining());

                        // Copy remaining bytes read in HTTP phase to input buffer used by
                        // frame processing

                        final int orgLimit = response.limit();
